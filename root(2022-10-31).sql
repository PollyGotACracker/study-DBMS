...
/*
인구가 1만 이상 5만 이하인 도시의 국가 정보를 같이 확인하고 싶다.
인구 정보는 city table 에 저장되어 있고
국가 정보는 country table 에 저장되어 있다.
이러한 상황이 되면 2개의 table 을 select 하여
데이터를 확인하고 확인된 데이터를 묶어주는 절차가 필요하다.
*/

-- 1 city 에서 인구가 1만 이상 5ㅁ나 이하는 데이터 SELECT
SELECT *
FROM city
WHERE Population BETWEEN 10000 AND 50000;
 
-- 2. SELECT 한 결과를 봤더니 국가에 대한 정보가 CountryCode 만 보여지고 있다.
-- CountryCode 를 가지고 다시 Country table 에서 국가 정보를 SELECT 해야 한다.
...
-- 완전 JOIN 주의 사항
-- city table 의 CountryCode 데이터가 "반드시" country table 의 Code 데이터에 존재해야 한다.
-- 만약 city 데이터에는 도시에 대한 정보가 있는데
-- 그 city table 의 CountryCode 에 해당하는 데이터가
-- country table 에 없으면 city 데이터의 리스트가 누락되어 버린다.
...
/*
INNER JOIN;일치 JOIN (EQ JOIN)
	두 table 의 교집합을 찾아 SELECT 하기
    두 테이블 간에 데이터가 완전 일치할 경우 사용
    	위의 SQL 문과 결과는 같다.
        
EQ JOIN : 완전 JOIN, INNER JOIN 을 말한다.
EQ JOIN 의 결과가 보증되려면 table 간에 "참조 무결성"이 보장되어야 한다. 
*/
...
LEFT OUTER JOIN, RIGHT OUTER JOIN... OUTER 는 생략 가능하다.
*/
